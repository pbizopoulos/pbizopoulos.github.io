...
int main(int argc, char *argv[]) {
    ...
    uint32_t event_mask;
    uint32_t window_geometry[4];

    xcb_keycode_t KEY_ALT_TAB = 23;
    xcb_keycode_t KEY_CTRL_ALT_DEL = 119;
    xcb_keycode_t KEY_CTRL_ALT_T = 28;
    ...
    window_geometry[0] = 0;
    window_geometry[1] = 0;
    window_geometry[2] = screen->width_in_pixels;
    window_geometry[3] = screen->height_in_pixels;

    xcb_grab_key(conn, 1, screen->root, XCB_MOD_MASK_1, KEY_ALT_TAB, XCB_GRAB_MODE_ASYNC, XCB_GRAB_MODE_ASYNC);
    xcb_grab_key(conn, 1, screen->root, XCB_MOD_MASK_1 | XCB_MOD_MASK_SHIFT, KEY_ALT_TAB, XCB_GRAB_MODE_ASYNC, XCB_GRAB_MODE_ASYNC);
    xcb_grab_key(conn, 1, screen->root, XCB_MOD_MASK_CONTROL | XCB_MOD_MASK_1, KEY_CTRL_ALT_T, XCB_GRAB_MODE_ASYNC, XCB_GRAB_MODE_ASYNC);
    xcb_grab_key(conn, 1, screen->root, XCB_MOD_MASK_CONTROL | XCB_MOD_MASK_1, KEY_CTRL_ALT_DEL, XCB_GRAB_MODE_ASYNC, XCB_GRAB_MODE_ASYNC);
    ...
    while (1) {
        ...
        switch (ev->response_type & ~0x80) {
            case XCB_KEY_PRESS: {
                ...
                if (kp->detail == KEY_ALT_TAB && (kp->state & XCB_MOD_MASK_1)) {
                    next_index = (kp->state & XCB_MOD_MASK_SHIFT) ? 
                                 current_client_index - 1 : 
                                 current_client_index + 1;
                    focus_client(conn, next_index);
                } else if (kp->detail == KEY_CTRL_ALT_DEL && (kp->state & (XCB_MOD_MASK_CONTROL | XCB_MOD_MASK_1))) {
                    break;
                } else if (kp->detail == KEY_CTRL_ALT_T && (kp->state & (XCB_MOD_MASK_CONTROL | XCB_MOD_MASK_1))) {
            ...
            case XCB_MAP_REQUEST: {
                ...
                xcb_configure_window(conn, clients[current_client_index],
                                     ...
                                     XCB_CONFIG_WINDOW_HEIGHT, window_geometry);
                break;
            }
