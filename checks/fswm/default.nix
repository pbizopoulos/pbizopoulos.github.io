...
pkgs.testers.runNixOSTest {
  ...
  testScript = ''
    ...
    with subtest("start xvfb and fswm"):
    ...
    with subtest("cycle focus across managed windows"):
      ...
      for _ in range(4):
        machine.succeed("DISPLAY=:1 xdotool key --window root Alt+Tab")
        seen.add(machine.succeed("DISPLAY=:1 xdotool getwindowfocus").strip())
      if not seen.issubset({w1, w2, w3, w4}) or len(seen) < 3:
        raise Exception(f"unexpected focus walk across managed windows: {sorted(seen)}")
      for _ in range(4):
        if machine.succeed("DISPLAY=:1 xdotool getwindowfocus").strip() == focus_before_cycle:
          break
        machine.succeed("DISPLAY=:1 xdotool key --window root Alt+Shift+Tab")
      else:
        raise Exception("failed to navigate focus back to the starting window")
      wait(f"test \"$(DISPLAY=:1 xdotool getwindowfocus)\" = \"{focus_before_cycle}\"")
      assert_in_stacking(focus_before_cycle, [w1, w2, w3, w4])
      for _ in range(4):
        if machine.succeed("DISPLAY=:1 xdotool getwindowfocus").strip() == w1:
          break
        machine.succeed("DISPLAY=:1 xdotool key --window root Alt+Tab")
      else:
